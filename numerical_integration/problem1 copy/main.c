...
{
    float pi=3.142857;
    float actual_value=8;
    
    printf("printing values for midpoint method\n");
    float actual1=fabs((midpoint(2,0,2*pi)-actual_value)/actual_value)*100;//actual error when n=2
    printf("actual error is: %f\n",actual1);
    //calculating actual and relative errors for powers of 2 starting from 4 till 1024.
    for(int i=2;i<11;i++)
    {
        
        float new=midpoint(pow(2,i),0,2*pi);
        float old=midpoint(pow(2,i-1),0,2*pi);
    ...
    float actual2=fabs((trapezoid(2,0,2*pi)-actual_value)/actual_value)*100;//actual error when n=2
    printf("actual error is: %f\n",actual2);
    //calculating actual and relative errors for powers of 2 starting from 4 till 1024.
    for(int i=2;i<11;i++)
    {

        float new=trapezoid(pow(2,i),0,2*pi);
        float old=trapezoid(pow(2,i-1),0,2*pi);
        float actual_error=fabs((new-actual_value)/actual_value)*100;
        float relative_error=fabs((new-old)/old)*100;
        printf("iteration : %d , actual error : %f ,relative error : %f\n",i-1,actual_error,relative_error);
    ...
    float actual3=fabs((simpson(2,0,2*pi)-actual_value)/actual_value)*100;//actual error when n=2
    printf("actual error is: %f\n",actual3);
    //calculating actual and relative errors for powers of 2 starting from 4 till 1024.
    for(int i=2;i<11;i++)
    {
        
        float new=simpson(pow(2,i),0,2*pi);
        float old=simpson(pow(2,i-1),0,2*pi);
        float actual_error=fabs((new-actual_value)/actual_value)*100;
        float relative_error=fabs((new-old)/old)*100;
        printf("iteration : %d , actual error : %f ,relative error : %f\n",i-1,actual_error,relative_error);
