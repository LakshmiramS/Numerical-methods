...
//this function should be used to find matrix inverse because we do not want the rows to be interchanged.
{
    for(int k=0;k<n-1;k++)
    {
...
int main()
{
    ...
    printf("\n");
    //define a b vector(useful to solve equations using matrix inverse
    float *b;
...
//    //return the contents of answer.
    ...
    //printing the values of the inverse matrix
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<n;j++)
        {
    ...
    printf("\n");
    //multiplying the inverse and the b vector to fill the answer vector
    for(int i=0;i<n;i++)
    {
        float sum=0;
        for(int j=0;j<n;j++)
